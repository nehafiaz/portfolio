...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div
        className={`max-w-7xl mx-auto flex items-center justify-between px-6 py-4 rounded-2xl border transition-all duration-500 ${scrolled
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <div className="flex items-center gap-4 pl-4 border-l border-border">
            ...
            <MagneticButton>
              <Link href="/work" className="px-6 py-2.5 bg-emerald-sig text-bg font-medium text-sm rounded-full hover:bg-emerald-sig/90 transition-colors">
                View Work
              </Link>
            </MagneticButton>
          </div>
        ...
        {/* Mobile menu toggle */}
        <button
          className="md:hidden text-text-1 relative z-[700] p-2"
          ...
          >
            {navLinks.map((link, i) => (
              <MotionLink
                key={link.name}
                href={link.href}
                className="text-2xl font-display text-text-1 hover:text-emerald-sig transition-colors"
